...
public class StringAnalyzer {
    ...
    public boolean setStringToAnalyze(String str) {
        ...
        for (int i = 0; i < listOfStrings.size(); i++) {
            ...
            String dumbVariableforDumbies = listOfStrings.get(i);
    ...
    public Character[] allMultipleChars() {
        ...
        for (int i = 0; i < this.currentString.length(); i++) {
            char ch1 = this.currentString.charAt(i);
            boolean isInArray = false;
            for (int x = 0; x < numFoundSoFar; x++) {
                char ch2 = arrayOfMultChars[x];
                if(ch1==ch2){
                    isInArray=true;
                }
            }
            if(!isInArray){
                ...
                for (int j = i+1; j < this.currentString.length(); j++) {
                    char ch2 = this.currentString.charAt(j);
                    if(ch1==ch2&&!multFound){
